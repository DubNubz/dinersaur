<template>
    <ion-page>
        ...
        <ion-toolbar>
            ...
            <ion-title>Restaurant Reviews</ion-title>
        </ion-toolbar>
        </ion-header>
...
onMounted(async () => {
const q = await getDoc(doc(db, "restaurants", userStore().userData?.uid ?? ""));

if (q) { 
    const userReviews = q.data()?.rating.personalizedRatings;
    if (userReviews) {
...
async function getName(id: string) {
    ...
    const data = q.data() as DocumentData;
    return data.name;
}
